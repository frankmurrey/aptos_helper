...
class ThalaAddLiquidity(ThalaLiquidityBase):
    ...
    def get_staked_lp_amount(
        ...
        if response.status_code != 200:
            logger.error(f"Error getting staked lp amount")
            return None
    ...
    def get_amount_out_for_token_pair(self) -> Union[dict, None]:
        ...
        if pool_data is None:
            ...
            coin_x = Tokens().get_by_contract_address(pool_data[0])
            coin_y = Tokens().get_by_contract_address(pool_data[1])
        ...
        if self.initial_balance_x_wei == 0 or self.initial_balance_y_wei == 0:
            logger.error(
                ...
                f"({wallet_x_balance_decimals} {coin_x.symbol.upper()},"
                f"{wallet_y_balance_decimals} {coin_y.symbol.upper()})"
            )
            return
...
class ThalaRemoveLiquidity(ThalaLiquidityBase):
    ...
    def get_amount_in_x_y(self, pool_data: list) -> Union[dict, None]:
        ...
        lp_ratio = self.get_lp_ratio(
            coin_x_address=pool_data[0],
            coin_y_address=pool_data[1],
    ...
    def build_txn_payload_data(self) -> Union[TransactionPayloadData, None]:
        ...
        lp_addr = f"0x48271d39d0b05bd6efca2278f22277d6fcc375504f9839fd73f74ace240861af::stable_pool::StablePoolToken" \
                  f"<{coin_x.contract_address}, {coin_y.contract_address}, {base_pool_address_x}, {base_pool_address_y}>"
        ...
        amount_in_x = amount_in_x_y.get(coin_x.contract_address)
        amount_in_y = amount_in_x_y.get(coin_y.contract_address)
        if amount_in_x is None or amount_in_y is None:
            logger.error(f"Error getting amount in")
            return None

        if coin_x.symbol != self.coin_x.symbol:
            token_x_decimals = self.token_y_decimals
            token_y_decimals = self.token_x_decimals
        ...
        payload = EntryFunction.natural(
            ...
            "remove_liquidity",
            [
                TypeTag(StructTag.from_str(pool_data[0])),
                TypeTag(StructTag.from_str(pool_data[1])),
                TypeTag(StructTag.from_str(base_pool_address_x)),
                TypeTag(StructTag.from_str(base_pool_address_y))
            ],
            transaction_args
        ...
        return TransactionPayloadData(
            payload=payload,
            amount_x_decimals=amount_in_x / 10 ** token_x_decimals,
            amount_y_decimals=amount_in_y / 10 ** token_y_decimals
        )
